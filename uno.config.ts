...
export default defineConfig({
	...
	shortcuts: {
		...
		// 图标
		"icon-wrapper":
			"inline-block leading-0 h-1em w-1em text-inherit font-not-italic normal-case text-center align-[-0.2em] text-optimizeLegibility antialiased [&_svg]:inline-block [&_svg]:align-baseline",
		// 登录背景波浪
		"bg-login-waves": "relative w-full h-20vh mb-[-7px] min-h-100px",
		"bg-login-wave-1":
			"animate-wave-move animate-delay-[-2s] animate-duration-[7s]",
		"bg-login-wave-2":
			"animate-wave-move animate-delay-[-3s] animate-duration-[10s]",
		"bg-login-wave-3":
			"animate-wave-move animate-delay-[-4s] animate-duration-[13s]",
		"bg-login-wave-4":
			"animate-wave-move animate-delay-[-5s] animate-duration-[20s]",
	},
	theme: {
		animation: {
			keyframes: {
				"wave-move":
					"{ 0% {transform: translate3d(-90px, 0, 0); } 100% {transform: translate3d(85px, 0, 0); }}",
			},
			durations: {
				"wave-move": "25s",
			},
